...
def genetic_algorithm_with_annealing(objective_function, bounds, num_vars, pop_size, n_generations, crossover_rate, mutation_rate, n_iterations_annealing, step_size, initial_temp, cooling_rate):
    ...
    for generation in range(n_generations):
        with ThreadPoolExecutor() as executor:
            fitness = list(executor.map(lambda individual: objective_function(individual, weight_f1, weight_f2), population))
        ...
        selected_indices = np.random.choice(np.arange(pop_size), size=pop_size, replace=True, p=fitness / np.sum(fitness))
        ...
        with ThreadPoolExecutor() as executor:
            population = list(executor.map(lambda individual: simulated_annealing(objective_function, individual, bounds, num_vars, n_iterations_annealing, step_size, initial_temp, cooling_rate)[0], next_population))
