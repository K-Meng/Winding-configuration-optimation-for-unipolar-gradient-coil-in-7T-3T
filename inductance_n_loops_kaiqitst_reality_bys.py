...
# def inductance_n_loops_kaiqitst_reality_bys(wbins, radius, wire_radius):
...
#     N1 = np.count_nonzero(wbins)
#
#     # 将 wbins 转换为 NumPy 数组以进行元素级操作
#     wbins = np.array(wbins)  # 添加这行代码
#
#     wbins = wbins - 1
#     wbins[wbins < 0] = 0
#     N2 = np.count_nonzero(wbins)
...
def inductance_n_loops_kaiqitst_reality_bys(wbins, radius, wire_radius):
    ...
    N1 = np.count_nonzero(wbins)
    wbins = np.array(wbins)  # 确保 wbins 是 NumPy 数组
    wbins = wbins - 1
    wbins[wbins < 0] = 0
    N2 = np.count_nonzero(wbins)
    N = N1 + N2

    # 限制 N 不超过 zspace_all 的大小
    N = min(N, len(zspace))
